...
import { API_BASE, buildWsUrl, getChats, createPrivateChat, createGroupChat, getApprovedPeers, getChatMessages, getReadState, setReadState, getPublicKey, publishPublicKey, addMembers, removeMembers, publishGroupKeyWrap, getGroupKeyWrap, getUnreadCounts, buildNotifyWsUrl, renameGroup, uploadEncryptedFile, sendMessageWithAttachment } from "../api";
import { getStoredPublicJwk, getSharedKeyWithUser, encryptTextAesGcm, decryptTextAesGcm, loadGroupKey, saveGroupKey, generateGroupKey, exportGroupKeyRaw, importGroupKeyRaw, encryptBytesAesGcm, decryptBytesAesGcm } from "@/lib/e2ee";
...
type Chat = { id: number; chat_type: string; participants: ChatParticipant[]; title?: string; name?: string | null; admin_user_id?: number | null };

export function ChatView({ token, onLogout }: Props) {
  const [chats, setChats] = useState<Chat[]>([]);
  const [activeChatId, setActiveChatId] = useState<number | null>(null);
  const [messages, setMessages] = useState<Array<{ id: number; content: string; sender?: { id: number; username: string }; timestamp?: string }>>([]);
  ...
  const [approvedUsers, setApprovedUsers] = useState<Array<{ user_id: number; username: string; chat_id?: number | null; is_self: boolean }>>([])
  ...
  const openNotifySocket = useCallback(() => {
    try {
      if (notifyWsRef.current && (notifyWsRef.current.readyState === WebSocket.OPEN || notifyWsRef.current.readyState === WebSocket.CONNECTING)) return
          ...
          if (data?.type === 'unread_update') {
            ...
            try {
              ...
              if (curr && data.chat_id === curr) {
                setMessages(prev => [...prev, { id: Date.now(), content: 'הוסרת מקבוצה זו', timestamp: new Date().toISOString() }])
              }
      ...
      nws.onerror = () => { try { nws.close() } catch {} }
    } catch {}
  }, [token])
  ...
  useEffect(() => {
    loadChats();
    openNotifySocket()
    return () => { try { notifyWsRef.current?.close() } catch {} }
  ...
  useEffect(() => {
    ...
    return () => { try { if (handler) window.removeEventListener('crypto-ready', handler) } catch {} }
  }, [token])
  ...
  const ensureGroupKey = useCallback(async (chat: Chat) => {
    if (chat.chat_type !== 'group') { groupKeyRef.current = null; return null }
    // try load existing
    const loaded = await loadGroupKey(chat.id)
    if (loaded) { groupKeyRef.current = loaded; return loaded }
    ...
    try {
      ...
      const shared = await getSharedKeyWithUser(wrap.provider_user_id, JSON.parse(providerKey.public_key_jwk))
      const raw = await decryptBytesAesGcm(wrap.wrapped_key_ciphertext, wrap.wrapped_key_nonce, shared)
    ...
    if (chat.admin_user_id && myId && chat.admin_user_id === myId) {
      ...
      for (const p of chat.participants) {
        ...
        try {
          ...
          const shared = await getSharedKeyWithUser(p.id, JSON.parse(pk.public_key_jwk))
          const wrapped = await encryptBytesAesGcm(raw, shared)
          await publishGroupKeyWrap(token, chat.id, p.id, wrapped.ciphertextB64, wrapped.nonceB64, wrapped.algo)
        } catch {}
      }
      return key
    }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
          ...
          if (chat.chat_type === 'private') {
            if (other) {
              try {
                ...
                const shared = await getSharedKeyWithUser(other.id, JSON.parse(keyRec.public_key_jwk))
                items = await Promise.all(history.map(async (m) => {
        ...
        setMessages(items.map(m => ({ id: m.id, content: m.content as string, sender: m.sender, timestamp: (m as any).timestamp as string })))
        try {
    ...
    ws.onopen = () => {
      ...
      for (const msg of pendingRef.current) {
        if (msg.chatId === activeChatId) {
          try { ws.send(JSON.stringify({ content: msg.text, content_type: "text" })); } catch {}
        } else {
          remain.push(msg)
        }
      }
    ...
    ws.onmessage = async (evt) => {
      try {
        ...
        if (data?.type === 'message' && data?.chat_id === activeChatId) {
          ...
          setMessages(prev => [...prev, { id: newId, content: content ?? '', sender: data.message?.sender, timestamp: ts }])
        ...
        setMessages((prev) => [...prev, { id: Date.now(), content: String(evt.data), timestamp: new Date().toISOString() }])
      }
    };
    ws.onclose = () => {
      wsRef.current = null;
    ...
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (ws && ws.readyState === WebSocket.OPEN) {
          try { ws.send(JSON.stringify({ content: text, content_type: "text" })); } catch {}
        } else {
  ...
  const unifiedItems = useMemo(() => {
    ...
    const privates = approvedUsers
      .map((u) => ({ kind: 'private' as const, userId: u.user_id, displayName: u.is_self ? "צ'אט עם עצמי" : u.username, chatId: u.chat_id ?? undefined }))
    ...
    <div className="w-screen h-screen overflow-hidden bg-white grid grid-cols-[320px_1fr] gap-4 min-h-0 box-border p-2">
      <aside className="border-l p-4 space-y-3 min-h-0 overflow-y-auto flex flex-col">
        <Card className="shrink-0">
          ...
          <CardContent className="p-2 space-y-2">
            <div className="flex gap-2 items-center">
              <Button variant={filterTab === 'all' ? 'default' : 'ghost'} onClick={() => setFilterTab('all')}>הכל</Button>
              <Button variant={filterTab === 'groups' ? 'default' : 'ghost'} onClick={() => setFilterTab('groups')}>קבוצות</Button>
            </div>
            <Input placeholder="חיפוש לפי שם משתמש או שם צ'אט" value={search} onChange={(e) => setSearch(e.target.value)} />
            <ul className="space-y-1">
              {unifiedItems.map((it) => {
                const key = (it as any).kind === 'group'
                  ? `group-${(it as any).chatId}`
                  : `private-${(it as any).userId}-${(it as any).chatId ?? 'new'}`
                return (
                <li key={key}>
                  <Button
        ...
        <div className="mt-auto">
          <Button className="w-full" variant="outline" onClick={onLogout}>יציאה</Button>
        </div>
      ...
      <main className="p-4 flex flex-col min-h-0">
        <ChatPanel
          title={(
            <>
              {(() => {
                ...
                const typeLabel = chat.chat_type === 'private' ? 'פרטי' : 'קבוצה';
                const baseTitle = (chat as any).title ? `${(chat as any).title} | ${typeLabel}` : (chat.chat_type === 'private' && myId && Array.isArray(chat.participants)) ? (() => {
                  const other = chat.participants.find((p: any) => p.id !== myId);
                  const name = other?.username ?? 'לא ידוע';
                  return `צ'אט עם ${name} | ${typeLabel}`;
                })() : `צ'אט עם ${chat.participants?.length ?? 0} משתתפים | ${typeLabel}`;
                return (
                  <span className="inline-flex items-center gap-2">
                    {baseTitle}
                    {chat.chat_type === 'group' && (
                      <button className="inline-flex items-center text-muted-foreground hover:text-foreground" onClick={() => setShowMembersDialog(true)} aria-label="פרטי קבוצה">
          ...
          footer={<ChatInput value={input} onChange={setInput} onSend={send} disabled={!activeChatId || (activeChatId != null && removedChatIds.has(activeChatId))} onSendCode={async (code, language) => {
            ...
            if (chat.chat_type === 'private') {
              ...
              if (other) {
                const keyRec = await getPublicKey(token, other.id)
                key = await getSharedKeyWithUser(other.id, JSON.parse(keyRec.public_key_jwk))
              }
            } else if (chat.chat_type === 'group') {
              key = await (loadGroupKey(chat.id) || ensureGroupKey(chat)) as any
              if (!(key instanceof CryptoKey)) key = await ensureGroupKey(chat)
            }
            ...
            const att = await uploadEncryptedFile(token, Uint8Array.from(atob(enc.ciphertextB64), c => c.charCodeAt(0)), file.name, file.type || 'application/octet-stream', enc.nonceB64)
            // Decide content type
        ...
        >
          <ChatMessages messages={messages} firstUnreadIndex={firstUnreadIndex} myId={myId} isGroup={(() => {
            const chat = chats.find((c: any) => c.id === activeChatId)
      ...
      <Dialog open={showGroupDialog} onOpenChange={(open) => {
        ...
        if (!open) { setGroupName(""); setSelectedUserIds(new Set()); setCreatingGroup(false) }
      }}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="max-h-60 overflow-y-auto border rounded-md p-2 space-y-1">
                {approvedUsers.filter(u => !u.is_self).map((u) => {
          ...
          <DialogFooter className="gap-2 sm:gap-2">
            <Button variant="outline" onClick={() => setShowGroupDialog(false)}>ביטול</Button>
            <Button disabled={creatingGroup || !groupName.trim()} onClick={async () => {
              if (creatingGroup) return
              setCreatingGroup(true)
              try {
                const ids = Array.from(selectedUserIds)
                const chat = await createGroupChat(token, ids, groupName.trim())
                await loadChats()
                setActiveChatId(chat.id)
      ...
      <Dialog open={showMembersDialog} onOpenChange={(open) => setShowMembersDialog(open)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
          ...
          {(() => {
            const chat = chats.find((c: any) => c.id === activeChatId)
            const isAdmin = Boolean(chat && (chat as any).chat_type === 'group' && (chat as any).admin_user_id === myId)
              ...
              <div className="space-y-3">
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    {isAdmin && (
                      <Button size="sm" disabled={renaming || !renameValue.trim()} onClick={async () => {
                        ...
                        setRenaming(true)
                        try {
                          await renameGroup(token, chat.id, renameValue.trim())
                          await loadChats()
                          setRenameValue("")
                        } catch {}
                        setRenaming(false)
                ...
                <div className="space-y-1">
                  ...
                  <ul className="max-h-48 overflow-y-auto border rounded-md p-2 space-y-1">
                    ...
                    {members.length === 0 && (
                      <li className="text-sm text-muted-foreground">אין משתתפים</li>
                    )}
                  </ul>
                </div>
                {isAdmin && (
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowMembersDialog(false)}>סגור</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
